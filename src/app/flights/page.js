...
function FlightsContent() {
  ...
  const calcularDetallesVuelo = async (flight) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/v1/flight-durations/duration?origen=${encodeURIComponent(flight.ruta.origen.nombre_ciudad)}&destino=${encodeURIComponent(flight.ruta.destino.nombre_ciudad)}`
      );
  ...
  }, [paramDestino, paramNombreCiudad]);

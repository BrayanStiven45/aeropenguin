...
export default function Home() {
  ...
  const [discountedFlights, setDiscountedFlights] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchCiudades = async () => {
      try {
        const response = await fetch(`${API_URL}/api/v1/ciudades`);
        if (response.ok) {
    ...
    // Cargar vuelo más barato y vuelos con descuento
    const fetchSpecialFlights = async () => {
      try {
        const response = await fetch(`${API_URL}/api/v1/flights`);
        if (response.ok) {
          const result = await response.json();
          if (result.success && result.data && result.data.length > 0) {
            const activeFlights = result.data.filter(f => f.estado === 1 && f.ruta);
            
            // Encontrar el vuelo más barato (segunda clase)
            if (activeFlights.length > 0) {
              const cheapest = activeFlights.reduce((min, flight) => {
                const currentPrice = flight.porcentaje_promocion 
                  ? flight.ruta.precio_segunda_clase * (1 - flight.porcentaje_promocion / 100)
                  : flight.ruta.precio_segunda_clase;
                const minPrice = min.porcentaje_promocion
                  ? min.ruta.precio_segunda_clase * (1 - min.porcentaje_promocion / 100)
                  : min.ruta.precio_segunda_clase;
                return currentPrice < minPrice ? flight : min;
              });
              setCheapestFlight(cheapest);
            }

            // Obtener vuelos con descuento
            const discounted = activeFlights.filter(f => f.porcentaje_promocion && f.porcentaje_promocion > 0);
            setDiscountedFlights(discounted.slice(0, 3)); // Máximo 3 vuelos con descuento
          }
        }
      } catch (error) {
        console.error('Error fetching special flights:', error);
      }
    };
    fetchSpecialFlights();
              ...
              <div key={ciudad.id_ciudad} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <div className="h-32 bg-gradient-to-r from-blue-400 to-purple-500 flex items-center justify-center text-4xl overflow-hidden relative">
                  ...
                  {ciudad.nombre_ciudad.toLowerCase() === "miami" && (
                    <>
                      <Image 
                        src={`${API_URL}/uploads/images/city/miami.jpg`} 
                        alt="Miami" 
                        fill
                  ...
                  {ciudad.nombre_ciudad.toLowerCase() === "new york" && (
                    <Image 
                      src={`${API_URL}/uploads/images/city/newyork.jpg`} 
                      alt="New York" 
                      fill
                      className="object-cover"
                    />
                  ...
                  {ciudad.nombre_ciudad.toLowerCase() === "madrid" && (
                    <>
                      <Image 
                        src={`${API_URL}/uploads/images/city/madrid.JPG`} 
                        alt="Madrid" 
                        fill
                        className="object-cover"
                        onError={(e) => { e.target.style.display = 'none'; }}
                  ...
                  {ciudad.nombre_ciudad.toLowerCase() === "londres" && (
                    <>
                      <Image 
                        src={`${API_URL}/uploads/images/city/londres.jpg`} 
                        alt="Londres" 
                        fill
                        className="object-cover"
                        onError={(e) => { e.target.style.display = 'none'; }}
      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            {/* Vuelo más barato */}
            {cheapestFlight && (
              <div className="bg-gradient-to-r from-green-400 to-emerald-600 rounded-xl p-8 text-white shadow-xl">
                <div className="flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-2xl font-bold">💰 Vuelo Más Económico</h3>
                  ...
                  <div className="bg-white/10 rounded-lg p-4 mb-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-lg">🛫 {cheapestFlight.ruta?.origen?.nombre_ciudad}</span>
                      <span className="text-2xl">✈️</span>
                      <span className="text-lg">🛬 {cheapestFlight.ruta?.destino?.nombre_ciudad}</span>
                    </div>
                    <div className="text-sm opacity-90">
                      📅 {new Date(cheapestFlight.fecha_vuelo).toLocaleDateString('es-ES')}
                    </div>
                  </div>
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-4xl font-bold">
                        ${cheapestFlight.porcentaje_promocion 
                          ? (cheapestFlight.ruta?.precio_segunda_clase * (1 - cheapestFlight.porcentaje_promocion / 100)).toFixed(2)
                          : cheapestFlight.ruta?.precio_segunda_clase}
                      </p>
                      {cheapestFlight.porcentaje_promocion > 0 && (
                    ...
                    <button 
                      className="bg-white text-green-600 font-bold py-3 px-6 rounded-lg hover:bg-gray-100 transition-colors shadow-md"
                      onClick={() => {
                        const params = new URLSearchParams({
                          origin: cheapestFlight.ruta?.ciudad_origen,
                          destination: cheapestFlight.ruta?.ciudad_destino,
                          departureDate: cheapestFlight.fecha_vuelo?.split('T')[0]
                        });
            ...
            <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-xl p-8 text-white shadow-xl">
              <div className="flex flex-col h-full">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="bg-white text-orange-600 px-3 py-1 rounded-full text-sm font-bold">
                    {discountedFlights.length} OFERTAS
                  </span>
                </div>
                
                {discountedFlights.length > 0 ? (
                  <div className="space-y-3 flex-grow">
                    {discountedFlights.slice(0, 2).map((flight) => (
                      <div key={flight.ccv} className="bg-white/10 rounded-lg p-3">
                        <div className="flex justify-between items-center mb-1">
                          <span className="font-semibold text-sm">
                            {flight.ruta?.origen?.nombre_ciudad} → {flight.ruta?.destino?.nombre_ciudad}
                          </span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                          <span>Desde ${(flight.ruta?.precio_segunda_clase * (1 - flight.porcentaje_promocion / 100)).toFixed(2)}</span>
                          <span className="opacity-75 line-through">${flight.ruta?.precio_segunda_clase}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <button 
                  className="w-full bg-white text-orange-600 font-bold py-3 px-6 rounded-lg hover:bg-gray-100 transition-colors shadow-md mt-4"
                  onClick={() => router.push('/flights?descuento=1')}
                >
